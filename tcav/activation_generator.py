...
class EEGActivationGenerator(ActivationGeneratorBase):
    def __init__(self, model, source_dir, acts_dir, max_examples=10):
        self.source_dir = source_dir #where EEG data is stored examples (concepts, class, random)
    ...
    def get_examples_for_concept(self, concept):
        ...
        #get paths for each eeg samples 
        eeg_paths_list = [os.path.join(concept_dir, d)
                     for d in tf.io.gfile.listdir(concept_dir)]
        #store eeg samples 
        #print(eeg_paths_list)
        eeg_exp = self.load_eegs_from_files(eeg_paths = eeg_paths_list, max_examples = self.max_examples) 

        return eeg_exp

    ##þarf ekki eins og er, nema við viljum seinna
    def load_eeg_from_file(self, filename): 
        """Given a filename, try to open the file. If failed, return None.
        Args:
          filename: location of the image file
          shape: the shape of the image file to be scaled
        Returns:
          the eeg  if succeeds, None if fails.
        Rasies:
          exception if the eeg was not the right shape. @TODO ??? maybe not a problem
        """
        #pass
    ...
    def load_eegs_from_files(self, eeg_paths, max_examples):
        ...
        for eeg in eeg_paths : 
            ...
            #print(eeg_tensor_new.size())
            if count == 0 : 
                theLength = eeg_tensor_new.size(2)
