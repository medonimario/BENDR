...
def find_inverse_time_window(tlen, annos4session):
    ...
    for i in range(0,len(annos4session) -1):
        t1 =  float(annos4session[3].iloc[i]) #get endtime of annotation i
        t2 = float(annos4session[2].iloc[i+1]) #get starttime of annotation i+1
        twanted = t2-t1
...
target = ['left']
concepts = ['eyem', 'musc'] #the labels that you want as concepts
# structore of TUAR data : 
# TUAR\edf\montage\patient\session\recording.edf
dataPath = '.\BENDR\get_data\TUAR\TUAR\edf\01_tcp_ar' #where raw data is stored 
mainPath = '.\BENDR\TCAV_folders_random_6s' #where we want to save folders 
