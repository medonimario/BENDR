...
def add_relative_amplitude_channel(epochs):
    """
    Add a 20th channel containing information about the relative amplitude.
    
    Args:
    - epochs (list): A list containing multiple MNE Epochs objects (each corresponding to a dataset)
    
    Returns:
    - modified_epochs_list (list): A list containing the MNE Epochs objects with the added channel
    """
    ...
    for epoch in epochs:
        ...
        for i in range(data.shape[0]):
            ...
            # Copy the existing data without normalization
            new_data[i, :-1] = data[i]
        ...
        modified_epoch = mne.EpochsArray(new_data, info, events=epoch.events, event_id=epoch.event_id)
        modified_epochs_list.append(modified_epoch)
    
    return modified_epochs_list
...
for pair_nr in pairs:
    ...
    # Add relative amplitude channel
    p1_uncoupled_epochs_19_norm, p1_coupled_epochs_19_norm, p2_uncoupled_epochs_19_norm, p2_coupled_epochs_19_norm = add_relative_amplitude_channel([p1_uncoupled_epochs_19, p1_coupled_epochs_19, p2_uncoupled_epochs_19, p2_coupled_epochs_19])
    ...
    df.to_csv(f"C:/1_University/Thesis/Scripts/BENDR/BENDR_half_norm_10Hz_av/pair{pair_nr}_df.csv", index=False)